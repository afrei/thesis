...
\textbf{Definition.} Given a model \(\mcM\) and an \(A \subset M\), we define the algebraic closure of \(A\), denoted \(\text{acl}(A)\) to be all elements of \(M\) which are in a finite set definable by an \(\mcL_A\)-formula. 
...
The algebraic closure has the nice properties that \(\text{acl}(\text{acl}(A)) = \text{acl}(A)\), that \(A \subset B \implies \text{acl}(A) \subset \text{acl}(B)\) and that if \(a \in \text{acl}(A)\) then there is a finite subset \(A_0 \subset A\) such that \(a \in \text{acl}(A_0)\).
