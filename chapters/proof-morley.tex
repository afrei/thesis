...
If \(\mcM \models T\) as above and \(A \subset M\), then \(\mcM\) is prime over \(A\) if, for all \(\mcN \models T\) and \(f:A \to \mcN\) partial elementary, we have a \(f^* \supset f\) embedding \(\mcM\) in \(\mcN\). 
...
That is, if \(T\) is \(\omega\)-stable, \(\mcM \models T\) and \(A \subset M\) then there is an \(\mcM_0\) which is prime over \(A\). 
...
We would like to show that if \(\mcM_0 \subset \mcM\) has universe \(A_\delta\) then \(\mcM_0 \prec \mcM\).
% Do that
...
\section{Algebraic Closure and Dimension}
