...
The type of \(v\) over \(M \cup \{c\}\) must be isolated by some formula \(\theta(v, c)\). (Consider the construction presented in Theorem \ref{lemma_minimal_omst}.)
...
Let \(\Gamma(c) \subseteq p(c)\) be these formulae just mentioned, that is \(\Gamma(c) = \{\A v \,(\theta(v, c) \to \delta_i(v)) \mid i \in \N\} \cup \{\E x \, \theta(x, c)\}\). 
...
As \(c'\) realizes \(\Gamma\), \(\mcM \models \A v' \,(\theta(v', c') \to \delta_i(v'))\) giving us that \(v'\) realizes \(\Delta\) in \(\mcM\). 
...
Therefore, \(\Gamma\) must be realized in \(\mcM\) and \(\Delta\) must be as well. 
\end{proof}
