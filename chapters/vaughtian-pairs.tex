...
Additionally, \(\A v \, (\theta(v, c) \to \delta_i(v)) \in p(c)\) for all \(i\). (As \(\theta(v, c)\) isolates the type of \(v\)). 
...
If \(c'\) realizes \(\Gamma\) in \(\mcM\) then \(\E x \, \theta(x, c')\) and any \(x\) for which \(\mcM \models \theta(x, c')\) is a realization of \(\Delta\) as desired. 
Therefore, it suffices to find a realization of \(\Gamma\) in \(\mcM\).
...
This shows that a countable type \(\Delta\) is realized in \(\mcM_0\) iff it is realized in \(\mcM\).  
\end{proof}
