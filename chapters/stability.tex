...
We will therefore prove the infinite Ramsey's theorem. 
It suffices to show for the case \(c = 2\) as we can divide the colors into two groups, apply the theorem and reduce the number of colors until we're left with only two. 
We will now induct on \(n\).
When \(n = 1\) we note the pidgeonhole principle tells us we cannot partition an infinite set into two finite parts. 
We now show the result \(n > 1\) assuming it is true for all values less than \(n\). 
Choose some arbitrary element \(s \in S\).
We induce a coloring of the \((n-1)\)-element subsets \(S'\) of \(S \setminus \{s\}\) by looking at the color of \(S' \cup \{s\}\).
We must have an infinite subset \(T'\) of \(S'\) whose size \(n-1\) subsets all have the same color. 
\textcolor{red}{Then some magic happens by wikipedia and stuff works out. \\ ...}  
...
%It suffices to show that for any \(n\) we can find infinitely many \(m \in M\) such that every ... 
