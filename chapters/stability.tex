...
Every formula is equivalent to a \qf one, so as these are all of the possible complete atomic types, they are also all of the possible types.    % More Detail
...
We can realize all of these in some elementary extension of \(\mcM\) which can be taken to be of cardinality exactly \(\kappa\) for any desired uncountable \(\kappa\). %4-3 Big Enough?
...
(The actual order of \(M\) will not matter, we fix an arbitrary one). %5-2 This is confusing
...
In our case, subsets of \(\Phi\) color subsets of size \(n\) taken from \(M\) by seeing which formulae of \(\Phi\) are true of the (ordered) elements of the subset. %More details
...
Let \(\mcM = \mathcal{H}((\kappa, <))\) be our model of cardinality \(\kappa\) and let \(A\) be an arbitrary countable subset of \(M\). % This is unclear
...
Therefore, any \(\omega\)-unstable theory cannot be categorical. %This section was sketchy
