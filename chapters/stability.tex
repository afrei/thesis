...
Still, neither \(v_1 = (s(0))\) or its negation is in \(p\) so \(p\) is not complete. 
...
To show that \(\Gamma\) is finitely satisfiable, it will suffice to show that sets of the form \(\Gamma_0 = T \cup \eldiag(\mcM) \cup \bigcup\limits_{\phi \in (p_i)_0} \phi(c_i)\) are satisfiable, where \((p_i)_0 \subseteq p_i\) are finite.  
\(\Gamma_0\) is satisfiable iff \(\Delta = T \cup \eldiag(\mcM) \cup \E x \bigwedge\limits_{\phi \in (p_i)_0} \phi(x)\) is satisfiable.
...
%It is natural to identify this set of order indiscernibles with \(I\), so that if \(\mcM \models \Gamma \cup T\), then \(I \subseteq M\).
...
Even though this last theorem was the main goal of this section, we introduce two more concepts which are connected with \(\omega\)-stability and prove some theorems about them. They are prime models and minimal formula. 
...
Otherwise, \(f_\alpha = \bigcup\limits_{\beta < \alpha}f_\beta\) and unions of partial elementary functions are partial elementary. 
