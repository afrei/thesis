...
We note that \(\mcM \prec \mcN\) then \(\mcM \subset \mcN\) (the inclusion map is simpmly given by sending \(a\) to the interpretation of the constant symbol \(a\) in \(\mcN\)).
...
We recall that an \(\mcL\)-theory, \(T\), is said to have built-in Skolem functions when for all \(\mcL\)-formulae \(\phi(v, \bar{w})\) is a function symbol \(f_\phi \in \mcL\) such that \(T \models \A \bar{w} \left[(\E v \, \phi(v, \bar{w})) \to \phi(f_\phi(\bar{w}), \bar{w})\right]\), in other words, the Skolem functions provide whitnesses for all existentials. 
\end{definition}
